...
export const SKILL_CONTENT = `# Comms CLI (tw)
...
tw auth token view               # Print the saved token to stdout (pipe-safe; refuses if COMMS_API_TOKEN is set)
...
Stored auth uses the system credential manager when available. If secure storage is unavailable, \`tw\` warns and falls back to \`~/.config/comms-cli/config.json\`. \`COMMS_API_TOKEN\` always takes priority over the stored token, and legacy plaintext config tokens are migrated automatically when secure storage is available.

In read-only mode (\`tw auth login --read-only\`), commands that modify Twist data (reply, archive, react, delete, etc.) are blocked by the CLI. Externally provided tokens (\`COMMS_API_TOKEN\` or \`tw auth token\`) are treated as unknown scope and assumed write-capable.

## View by URL

\`\`\`bash
tw view <url>                    # View any Comms entity by URL
\`\`\`
...
- **Twist URLs**: Full \`https://comms.todoist.com/...\` URLs (parsed automatically)
...
\`\`\`bash
tw view https://comms.todoist.com/a/1585/ch/100/t/200          # View thread
tw view https://comms.todoist.com/a/1585/ch/100/t/200/c/300     # View comment
tw view https://comms.todoist.com/a/1585/msg/400                 # View conversation
tw view https://comms.todoist.com/a/1585/msg/400/m/500 --json    # View message as JSON
\`\`\`
