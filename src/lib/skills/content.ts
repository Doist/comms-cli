...
export const SKILL_CONTENT = `# Comms CLI (cm)
...
Stored auth uses the system credential manager when available. If secure storage is unavailable, \`cm\` warns and falls back to \`~/.config/comms-cli/config.json\`. \`COMMS_API_TOKEN\` always takes priority over the stored token.
...
--accessible               # Add text labels to color-coded output (also: CM_ACCESSIBLE=1)
