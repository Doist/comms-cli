 ...
 */
export function createCommsUserRecordStore(): UserRecordStore<CommsAccount> {
    return {
        async list() {
 ...
 */
export function getDefaultUserRecord(config: Config): UserRecord<CommsAccount> | null {
...
function toRecord(user: StoredUser): UserRecord<CommsAccount> {
    const account = makeCommsAccount({
        id: user.id,
        label: user.name,
...
function fromRecord(record: UserRecord<CommsAccount>): StoredUser {
