...
describe('ProgressTracker', () => {
    ...
    describe('initialization and enabling', () => {
        it.each([
            ['disabled by default', ['node', 'cm', 'threads'], false],
            [
                'enabled with --progress-jsonl flag',
                ['node', 'cm', 'threads', '--progress-jsonl'],
                true,
            ],
            [
                'enabled with --progress-jsonl=path flag',
                ['node', 'cm', 'threads', '--progress-jsonl=/tmp/progress.jsonl'],
                true,
            ],
            [
    ...
    describe('output destinations', () => {
        it('should output to stderr by default', () => {
            process.argv = ['node', 'cm', 'threads', '--progress-jsonl']
            const tracker = new ProgressTracker()
        ...
        it('should create file when path is provided with equals', () => {
            process.argv = ['node', 'cm', 'threads', '--progress-jsonl=/tmp/progress.jsonl']
            const tracker = new ProgressTracker()
    ...
    describe('event emission', () => {
        ...
        it('should not emit events when disabled', () => {
            resetProgressTracker()
            resetGlobalArgs()
            process.argv = ['node', 'cm', 'threads'] // No --progress-jsonl flag
...
describe('edge cases and integration', () => {
    ...
    it.each([
        ['flag in middle of arguments', ['node', 'cm', '--progress-jsonl', 'threads', '--json']],
        ['flag at end of arguments', ['node', 'cm', 'threads', '--json', '--progress-jsonl']],
        ['flag with empty path argument', ['node', 'cm', 'threads', '--progress-jsonl', '']],
        ['flag followed by another flag', ['node', 'cm', 'threads', '--progress-jsonl', '--json']],
