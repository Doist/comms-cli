...
describe('away', () => {
    ...
    describe('show', () => {
        it('shows not away when awayMode is null', async () => {
            ...
            await program.parseAsync(['node', 'cm', 'away'])
    ...
    describe('set', () => {
        it('calls users.update with awayMode', async () => {
            ...
            await program.parseAsync(['node', 'cm', 'away', 'set', 'vacation', '2026-03-20'])
        ...
        it('propagates insufficient scope errors (handled globally by API proxy)', async () => {
            const scopeError = new CommsRequestError('Request failed with status 403', 403, {
                error_code: 109,
            ...
            await expect(
                program.parseAsync(['node', 'cm', 'away', 'set', 'vacation', '2026-03-20']),
        ...
        it('rejects invalid away type', async () => {
            ...
            await expect(
                program.parseAsync(['node', 'cm', 'away', 'set', 'invalid', '2026-03-20']),
    ...
    describe('clear', () => {
        it('calls users.update with empty string awayMode to clear', async () => {
            ...
            await program.parseAsync(['node', 'cm', 'away', 'clear'])
        ...
        it('shows dry-run message', async () => {
            ...
            await program.parseAsync(['node', 'cm', 'away', 'clear', '--dry-run'])
