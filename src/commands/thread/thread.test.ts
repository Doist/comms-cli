...
describe('thread read', () => {
    ...
    it('reads thread refs from --from-file', async () => {
        const dir = await mkdtemp(join(tmpdir(), 'tdc-mark-read-'))
        const refsPath = join(dir, 'ids.txt')
        await writeFile(refsPath, '# comment\n500\n\n')
