...
describe('installer paths', () => {
    const cases = [
        { agent: 'claude-code', dir: '.claude', desc: 'Claude Code skill for Comms CLI' },
        { agent: 'codex', dir: '.codex', desc: 'Codex skill for Comms CLI' },
        { agent: 'cursor', dir: '.cursor', desc: 'Cursor skill for Comms CLI' },
        { agent: 'gemini', dir: '.gemini', desc: 'Gemini CLI skill for Comms CLI' },
        { agent: 'pi', dir: '.pi', desc: 'Pi skill for Comms CLI' },
        { agent: 'universal', dir: '.agents', desc: 'Universal agent skill for Comms CLI' },
    ] as const
    ...
    for (const { agent, dir, desc } of cases) {
        describe(agent, () => {
            ...
            it(`returns global path containing ${dir}/skills`, () => {
                ...
                expect(globalPath).toContain('comms-cli')
            ...
            it('returns local path containing cwd', () => {
                ...
                expect(localPath).toContain('comms-cli')
...
describe('installer operations', () => {
    ...
    beforeEach(async () => {
        testDir = join(tmpdir(), `comms-cli-test-${Date.now()}`)
...
describe('skill command', () => {
    ...
    it('lists agents', async () => {
        ...
        await program.parseAsync(['node', 'tdc', 'skill', 'list', '--local'])
        expect(consoleSpy).toHaveBeenCalledWith(expect.stringContaining('Available agents'))
    })

    it('installs agent locally', async () => {
        const program = createProgram()
        await program.parseAsync(['node', 'tdc', 'skill', 'install', 'claude-code', '--local'])
    ...
    it('installs codex agent locally', async () => {
        const program = createProgram()
        await program.parseAsync(['node', 'tdc', 'skill', 'install', 'codex', '--local'])
        expect(consoleSpy).toHaveBeenCalledWith('✓', expect.stringContaining('Installed'))
    ...
    it('installs cursor agent locally', async () => {
        const program = createProgram()
        await program.parseAsync(['node', 'tdc', 'skill', 'install', 'cursor', '--local'])
        expect(consoleSpy).toHaveBeenCalledWith('✓', expect.stringContaining('Installed'))
    ...
    it('uninstalls agent', async () => {
        ...
        const program = createProgram()
        await program.parseAsync(['node', 'tdc', 'skill', 'uninstall', 'claude-code', '--local'])
    ...
    it('errors on unknown agent', async () => {
        ...
        await expect(
            program.parseAsync(['node', 'tdc', 'skill', 'install', 'unknown-agent', '--local']),
    ...
    it('updates a specific agent', async () => {
        ...
        const program = createProgram()
        await program.parseAsync(['node', 'tdc', 'skill', 'update', 'claude-code', '--local'])
    ...
    it('errors on update of unknown agent', async () => {
        ...
        await expect(
            program.parseAsync(['node', 'tdc', 'skill', 'update', 'unknown-agent', '--local']),
        ).rejects.toHaveProperty('code', 'UNKNOWN_AGENT')
    })
    ...
    it('updates all installed agents', async () => {
        ...
        const program = createProgram()
        await program.parseAsync(['node', 'tdc', 'skill', 'update', 'all', '--local'])
    ...
    it('defaults to all when no agent specified', async () => {
        ...
        const program = createProgram()
        await program.parseAsync(['node', 'tdc', 'skill', 'update', '--local'])
...
describe('install detection', () => {
    ...
    it('skips agent directory check for universal (.agents)', async () => {
        const testDir = join(tmpdir(), `comms-cli-universal-test-${Date.now()}`)
        await mkdir(testDir, { recursive: true })
        const originalCwd = process.cwd()
        process.chdir(testDir)
        try {
